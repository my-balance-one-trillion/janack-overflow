<template>
  <div v-if="Object.keys(nowAccount).length > 0"
    ...
    <div class="flex-col py-3 text-sm min-w-fit">
      <div class="flex rounded-[20px] w-fit px-8 h-10 bg-sub-red text-white  justify-center items-center my-1">
        DAY {{ nowAccount.dayCnt }}
      </div>
      <div class="flex rounded-[20px] w-fit px-7 h-10 bg-sub-red text-white justify-center items-center">
        TOTAL {{ nowAccount.savingCnt }}
      </div>
    </div>
    <span style="font-size: 51px">{{ nowAccount.acntName }}</span>
    <div class="flex flex-col justify-evenly h-1/2 min-w-fit w-7/12 mx-0 rounded-b-[25px]">
    ...
    <div
      ...
      <Chart v-if="Object.keys(nowAccount).length > 0" :goalAmount="nowAccount.goalAmount"
  ...
  <div class="flex justify-end w-9/12 mx-auto">
    <button
      class="rounded-[25px] bg-main-red hover:bg-hover-red text-white w-fit h-fit px-5 py-3"
  ...
  <div class="border rounded-[25px] w-9/12 mx-auto my-10 p-4">
    <div class="flex justify-between mx-4">
      <span style="font-size: 38px">나의 최근 <span class="text-main-red">에러</span></span>
      <router-link :to="'/mypage/myissue'">
        <span class="underline text-main-red" style="font-size: 21px; line-height: 200%">내역 조회</span>
      </router-link>
    </div>
