<template>
  <div
    ...
    class="px-4 flex flex-col rounded-[25px] border border-grey border-solid my-10 h-[610px] w-9/12 mx-auto justify-between relative shadow-md"
    style="background-image: linear-gradient(to bottom, white 50%, #bf1131 50%)"
  >
    <div class="flex-col min-w-fit py-3">
      <div
    ...
    <div
      class="sm:hidden hidden md:flex flex-col absolute border bg-white rounded-[25px] w-4/12 my-24 right-6 items-center"
    >
      <div class="text-left" style="font-size: 31px">목표</div>
      <div class="text-center" style="font-size: 31px">
        {{ nowAccount.goalName }}
      </div>
      <Chart
        v-if="Object.keys(nowAccount).length > 0"
  ...
  <div v-else class="w-9/12 mx-auto flex justify-end">
    <button
      class="rounded-[25px] bg-main-red text-white w-[180px] h-[70px]"
  ...
  <div
    class="border rounded-[25px] w-9/12 mx-auto my-10 p-4"
  >
    <div class="flex justify-between mx-4">
      ...
      <router-link :to="'/mypage/myissue'">
        <span
          ...
          style="font-size: 31px; line-height: 300%"
          >내역 조회</span
        >
      </router-link>
    </div>
